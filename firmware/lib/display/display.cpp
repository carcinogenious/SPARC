...
void display::init() {
    ...
    // though the SSD1306 init and RST pulse below run without error. Give
    // the rail a moment to settle before resetting the controller.
    pinMode(Vext, OUTPUT);
...
void display::flightSummary(float peakAlt, float hoverTime, float maxTilt,
                            float landingSpeed, float co2Used) {
    oled.clear();
    oled.setFont(ArialMT_Plain_10);
    oled.drawString(0, 0,  String("Peak  ") + String(peakAlt, 2) + " m");
    oled.drawString(0, 11, String("Hover ") + String(hoverTime, 1) + " s");
    oled.drawString(0, 22, String("Tilt  ") + String(maxTilt, 1) + " deg");
    oled.drawString(0, 33, String("Land  ") + String(landingSpeed, 2) + " m/s");
    oled.drawString(0, 44, String("CO2   ") + String(co2Used, 1) + " g");
    oled.display();
}
