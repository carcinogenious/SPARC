// Attitude estimator — SPARC flight controller (sensor fusion)
// Complementary filter fusing the MPU6050 gyro (fast, immune to linear
// acceleration, but drift-prone) with the accelerometer's gravity vector
// (slow, noisy, but absolute) into signed pitch and roll. This is the
// attitude the TVC PID stabilizes and the angle the ToF slant range is
// tilt-corrected with.
#pragma once
